...
The `Syde\Dbal\Schema\Columns` implementation groups multiple `Syde\Dbal\Schema\Column` instances. This is required when working [Schemas](./schemas.md).
...
`syde/dbal` brings an implementation for columns which are used in a Schema to create a database table. The main class for this is `Syde\Dbal\Schema\Column` which provides a wide range of methods:
...
Additionally to the default formats, `Syde\Dbal\Schema\Column` also provides some pre-configured typical formats:
...
`Syde\Dbal\Schema\Column` additionally supports attributes for the data types.
